...
class OBCAOptimizer:
    ...
    def initialize(self, init_guess, obs, max_x, max_y):
        self.init_state = SX([init_guess[0].x, init_guess[0].y, init_guess[0].v, init_guess[0].heading, init_guess[0].steer])
        self.end_state = SX([init_guess[-1].x, init_guess[-1].y, init_guess[-1].v, init_guess[-1].heading, init_guess[-1].steer])
    ...
    def generate_variable(self):
        ...
        for i in range(self.N):
            ...
            self.lbx += [0, 0, -self.v_cfg.max_v, -2 * pi, -self.v_cfg.max_front_wheel_angle]
