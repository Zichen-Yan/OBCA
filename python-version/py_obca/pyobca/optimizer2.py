...
class OBCAOptimizer:
    def __init__(self, cfg: VehicleConfig = VehicleConfig()) -> None:
        ...
        self.x0 = []
        self.obstacles = []
        self.G = DM([[1, 0], [-1, 0], [0, 1], [0, -1], ])
        self.g = vertcat(SX([cfg.length / 2, cfg.length / 2, 0.5 * cfg.width, 0.5 * cfg.width]))
        self.T = cfg.T
        self.kappa = 1

    def initialize(self, init_guess, obs, max_x, max_y):
        self.init_state = SX([init_guess[0].x, init_guess[0].y, init_guess[0].heading, init_guess[0].v, init_guess[0].steer])
        self.end_state = SX([init_guess[-1].x, init_guess[-1].y, init_guess[-1].heading, init_guess[-1].v, init_guess[-1].steer])
        print(init_guess[-1].__dict__)
        self.N = len(init_guess)

        self.obstacles = obs
        for state in init_guess:
            self.x0 += [[state.x, state.y, state.heading, state.v, state.steer]]
        self.x0 += [[0] * (self.n_controls * (self.N - 1))]
        self.x0 += [[0.1] * (self.n_dual_variable * (self.N) * 2 * len(self.obstacles))]
        self.x0 += [[0] * (self.N * len(self.obstacles))]
    ...
    def build_model(self) -> bool:
        ...
        self.state = vertcat(x, y, theta, v, steering)
        self.control = vertcat(a, steering_rate)
        self.rhs = vertcat(v * cos(theta), v * sin(theta), v / (self.lr + self.lf) * tan(steering), steering_rate, a)
        ...
        self.MU = SX.sym('MU', self.n_dual_variable, self.N * len(self.obstacles))
        self.LAMBDA = SX.sym('LAMBDA', self.n_dual_variable, self.N * len(self.obstacles))
        self.s = SX.sym('SOFT', 1, self.N * len(self.obstacles))
        self.obj = 0

        return True

    def solve(self):
        nlp_prob = {'f': self.obj, 'x': vertcat(*self.variable), 'g': vertcat(*self.constrains)}
        solver = nlpsol('solver', 'ipopt', nlp_prob)
        sol = solver(x0=vertcat(*self.x0), lbx=vertcat(*self.lbx), ubx=vertcat(*self.ubx), ubg=vertcat(*self.ubg), lbg=vertcat(*self.lbg))
        u_opt = sol['x']
        self.x_opt = u_opt[0:self.n_states * (self.N):self.n_states]
        self.y_opt = u_opt[1:self.n_states * (self.N):self.n_states]
        self.theta_opt = u_opt[2:self.n_states * (self.N):self.n_states]
        self.v_opt = u_opt[3:self.n_states * (self.N):self.n_states]
        self.steer_opt = u_opt[4:self.n_states * (self.N):self.n_states]
        self.a_opt = u_opt[self.n_states * (self.N):self.n_states * (self.N) + self.n_controls * (self.N - 1):self.n_controls]
        self.steerate_opt = u_opt[self.n_states * (self.N) + 1:self.n_states * (self.N) + self.n_controls * (self.N - 1):self.n_controls]
    ...
    def generate_object(self, r, q):
        ...
        for i in range(self.N - 1):
            ...
            for j in range(len(self.obstacles)):
                self.obj -= (self.kappa * self.s[:, len(self.obstacles) * i + j])

    def generate_variable(self):
        # state constraints
        for i in range(self.N):
            self.variable += [self.X[:, i]]
            self.lbx += [0, 0, -2 * pi, -self.v_cfg.max_v, -self.v_cfg.max_front_wheel_angle]
            self.ubx += [self.max_x, self.max_y, 2 * pi, self.v_cfg.max_v, self.v_cfg.max_front_wheel_angle]
        # input constraints
        for i in range(self.N - 1):
    ...
    def generate_constrain(self):
        ...
        for i in range(self.N):
            ...
            for obstacle in self.obstacles:
                ...
                x = st[0]
                y = st[1]
                t = vertcat(x + self.offset * cos(heading),  # x,y表示后轴中心位置，算上offset后表示车辆的正中点
                ...
                soft = vertcat(self.s[:, len(self.obstacles) * i + index])
                index += 1
                ...
                self.lbg += [0, 0]
                self.ubg += [0, 0]
                self.constrains += [(-dot(self.g, mu) + dot(A @ t - b, lamb)) - soft]
                self.lbg += [0]
                self.ubg += [100000]
