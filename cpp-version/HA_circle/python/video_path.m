...
img = getframe(gcf); % 从当前figure框中获取视频帧
writeVideo(videoFWriter,img);
plot(path(:,1)*0.01,path(:,2)*0.01,'r');
% plot(ori_path(:,1)*0.01,ori_path(:,2)*0.01,'k');
img = getframe(gcf); % 从当前figure框中获取视频帧
writeVideo(videoFWriter,img);
...
radius8 = sqrt((wid/4)^2+(length/8)^2); 
...
radius3 = sqrt((wid/2)^2+(length/6)^2)-0.1; 
...
% radius2 = sqrt((wid/2)^2+(length/4)^2)-0.2; 
% x = radius2 * cos(theta); % x 坐标
% y = radius2 * sin(theta); % y 坐标
% c21 = plot(x, y, 'b');
% c22 = plot(x, y, 'b');
...
%     h2.YData = CAR_1(2,:);
    ...
    cosphi=cos(path(index,3)*pi/180);
    sinphi=sin(path(index,3)*pi/180);
    cpx=path(index,1)*0.01;
    cpy=path(index,2)*0.01;
    rect_x=length/2-(0.99+0.2);
    % 8个圆
    rect_y=wid/4;
    offset=length/8;
    ...
    rect_y=-wid/4;
    center_x5 = (rect_x+3*offset-0.2) * cosphi - (rect_y+0.28) * sinphi + cpx;
	center_y5 = (rect_x+3*offset-0.2) * sinphi + (rect_y+0.28) * cosphi + cpy;
    center_x6 = (rect_x+offset) * cosphi - (rect_y+0.15) * sinphi + cpx;
	center_y6 = (rect_x+offset) * sinphi + (rect_y+0.15) * cosphi + cpy;
    center_x7 = (rect_x-offset) * cosphi - (rect_y+0.15) * sinphi + cpx;
	center_y7 = (rect_x-offset) * sinphi + (rect_y+0.15) * cosphi + cpy;
    center_x8 = (rect_x-3*offset+0.2) * cosphi - (rect_y+0.28) * sinphi + cpx;
	center_y8 = (rect_x-3*offset+0.2) * sinphi + (rect_y+0.28) * cosphi + cpy;

    radius = radius8;
    c81.XData = center_x1 + (radius+0.15) * cos(theta); % x 坐标
    c81.YData = center_y1 + (radius+0.15) * sin(theta); % y 坐标
    ...
    c84.XData = center_x4 + (radius+0.15) * cos(theta);
    c84.YData = center_y4 + (radius+0.15) * sin(theta); 
    c85.XData = center_x5 + (radius+0.15) * cos(theta);
    c85.YData = center_y5 + (radius+0.15) * sin(theta); 
    ...
    c88.XData = center_x8 + (radius+0.15) * cos(theta);
    c88.YData = center_y8 + (radius+0.15) * sin(theta); 
    % 3个圆
    rect_y=0;
    offset=length/3;
    ...
    center_x2 = (rect_x) * cosphi + cpx;
	center_y2 = (rect_x) * sinphi + cpy;
    center_x3 = (rect_x-offset+0.25) * cosphi + cpx;
	center_y3 = (rect_x-offset+0.25) * sinphi + cpy;
...
%     offset=length/4;
% 
%     center_x1 = (rect_x+offset-0.15) * cosphi + cpx;
%     center_y1 = (rect_x+offset-0.15) * sinphi + cpy;
%     center_x2 = (rect_x-offset+0.15) * cosphi + cpx;
% 	center_y2 = (rect_x-offset+0.15) * sinphi + cpy;
%     
%     raidus=radius2;    
%     c21.XData = center_x1 + raidus * cos(theta); % x 坐标
%     c21.YData = center_y1 + raidus * sin(theta); % y 坐标
%     c22.XData = center_x2 + raidus * cos(theta);
%     c22.YData = center_y2 + raidus * sin(theta);
