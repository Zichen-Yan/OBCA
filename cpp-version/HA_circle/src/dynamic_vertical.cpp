...
double road_find_first(double x_now, double y_now, double thea_now, double x_aim, double y_aim, double thea_aim, double y_tolerance, double min_r, double yaw_tolerance, double road_yaw[8]);
int RoadCollision(double Start[3], double End[3], double road_yaw[8]);
void getroadpath(double Start[3], double road_yaw[8]);
char road_find_three(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_R_S[8]);
char angleget(double sin_result, double cos_result, double *last_angle_result);
char road_find_two(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_two[8]);
char road_find_four(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double max_x, double road_yaw[8]);
int CCS_five(double x, double y, double phi, RsPath &path_five, int theta_car);
...
int CCS_six(double x, double y, double phi, RsPath &path_six, int theta_car);
char road_find_six(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_yaw[8], double road_yaw2[8]);
int CCS_seven(double x, double y, double phi, RsPath &path_seven, int theta_car);
char road_find_three_cs(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double bias, double *road_R_S);
...
int CCS_five(double x, double y, double phi, RsPath &path_five, int theta_car)
{
	...
	{
		path_five.pathlength[0] = t;
		path_five.pathlength[1] = u;
		path_five.pathlength[2] = v;
		path_five.pathlength[3] = 0;
		path_five.pathlength[4] = 0;
		path_five.lenth = fabs(t) + fabs(u) + fabs(v);
		path_five.pathtype = "LRS";
		return 0;
	}
	if (LpRpSp(-x, y, -phi, t, u, v) && (((t <= 0) && (u <= 0) && (v >= 0) && (theta_car == 1)) || ((t >= 0) && (u >= 0) && (v <= 0) && (theta_car == -1))))
...
int CCS_six(double x, double y, double phi, RsPath &path_six, int theta_car)
{
	...
	{
		path_six.pathlength[0] = t;
		path_six.pathlength[1] = u;
		path_six.pathlength[2] = v;
		path_six.pathlength[3] = 0;
		path_six.pathlength[4] = 0;
		path_six.lenth = fabs(t) + fabs(u) + fabs(v);
		path_six.pathtype = "LRS";
		return 0;
	}
	if (LpRpSp(-x, y, -phi, t, u, v) && (((t <= 0) && (u >= 0) && (v >= 0) && (theta_car == 1)) || ((t >= 0) && (u <= 0) && (v <= 0) && (theta_car == -1))))
	//	if (LpRpSp(-x, y, -phi, t, u, v) && (t <= 0) && (u >= 0) && (v >= 0)) // timeflip
...
int CCS_seven(double x, double y, double phi, RsPath &path_seven, int theta_car)
{
	...
	if (LpRpSp(x, y, phi, t, u, v) && (((t <= 0) && (u >= 0) && (v <= 0) && (theta_car == 1)) || ((t >= 0) && (u <= 0) && (v >= 0) && (theta_car == -1))))
	{
		path_seven.pathlength[0] = t;
		path_seven.pathlength[1] = u;
		path_seven.pathlength[2] = v;
		path_seven.pathlength[3] = 0;
		path_seven.pathlength[4] = 0;
		path_seven.lenth = fabs(t) + fabs(u) + fabs(v);
		path_seven.pathtype = "LRS";
		return 0;
	}
	if (LpRpSp(-x, y, -phi, t, u, v) && (((t >= 0) && (u <= 0) && (v >= 0) && (theta_car == 1)) || ((t <= 0) && (u >= 0) && (v <= 0) && (theta_car == -1))))
	{
...
double road_find_first(double x_now, double y_now, double thea_now, double x_aim, double y_aim, double thea_aim, double y_tolerance, double min_r, double yaw_tolerance, double road_yaw[8]) // 根据车辆当前点及目标点计算转弯半径
{
...
// return 0代表有解
char road_find_two(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_two[6])
{
	//////////////////////////////车辆坐标转换处理/////////////////////////////////////////////////////////////////////
	double y = (y_aim - y_now) * cos(theta_now) - (x_aim - x_now) * sin(theta_now); // 坐标转换后 Y向的实际偏差
	double x = (x_aim - x_now) * cos(theta_now) + (y_aim - y_now) * sin(theta_now);
	...
	if (a == 0)
	{
		...
		if (angleget(sin_theta_first_1, cos_theta_first_1, angle_1) != 0) // 无解
		{
			return 1;
		}
		////////////////////可删除下面两行验证用/////////////////////////////////////////
	...
	if (fabs(R_1) >= car_r_limit)
	{
		sin_theta_first_1 = x / 2 / (R_1) + sin(theta) / 2;
		cos_theta_first_1 = -y / 2 / (R_1) + cos(theta) / 2 + 0.5;
		if (angleget(sin_theta_first_1, cos_theta_first_1, angle_1) != 0)
		{
			return 1;
		}
	...
	if (fabs(R_2) >= car_r_limit)
	{
		...
		if (angleget(sin_theta_first_2, cos_theta_first_2, angle_2) != 0)
		{
			return 1;
		}
...
char road_find_four(double x_now, double y_now, double theta_now,
					...
					double car_r_limit, double bias, double *road_R_S)
{
	...
	if (car_r_limit_m * (-theta_rad) * x_1 > 0)				  // 圆弧的解需换挡
	{
		if (y * y_1 < 0) // 最小转弯半径也照样调整不到位,一个在y=0的左边，一个在右边
		{
			if (fabs(y_1) < bias && fabs(x_1) <= 6) // 是否在误差允许范围内 并且未超出对向限制空间
			{
				road_R_S[0] = car_r_limit_m;
				road_R_S[1] = car_r_limit_m * (-theta_rad);
				road_R_S[2] = 1000000;
...
char road_find_five(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_yaw[8], double road_yaw2[8])
{
	...
	if (fabs(ph) < 45 * pi / 180)
	{
		...
		ErrorCode = CCS_five(x1_five, y1_five, ph, path_five, theta_car);
		if (ErrorCode == 0)
		{
			int num = path_five.pathtype.size();
			for (int index_five_CCS = 0; index_five_CCS < num; index_five_CCS++)
			{
				if (path_five.pathtype[index_five_CCS] == 'S')
				{
					road_yaw[index_five_CCS * 2] = 1000000;
				}

				else if (path_five.pathtype[index_five_CCS] == 'L')
				{
					road_yaw[index_five_CCS * 2] = rmin;
				}

				else if (path_five.pathtype[index_five_CCS] == 'R')
				{
					road_yaw[index_five_CCS * 2] = -rmin;
				}
			}

			road_yaw[1] = path_five.pathlength[0] * rmin;
			road_yaw[3] = path_five.pathlength[1] * rmin;
			road_yaw[5] = path_five.pathlength[2] * rmin;
		...
		roadthird_five = road_find_two(0, 0, 0, x, y, theta, R_V, road_yaw_R);
		if (roadthird_five == 0)
		{
...
char road_find_six(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_yaw[8], double road_yaw2[8])
{
	...
	fflush(bydapa::common::Log::GetInstance()->fileptr());
	ErrorCode = CCS_six(x1_six, y1_six, ph, path_six, theta_car);
	if (ErrorCode == 0)
	{
		int num = path_six.pathtype.size();
		for (int index_six_CCS = 0; index_six_CCS < num; index_six_CCS++)
		{
			if (path_six.pathtype[index_six_CCS] == 'S')
			{
				road_yaw[index_six_CCS * 2] = 1000000;
			}

			else if (path_six.pathtype[index_six_CCS] == 'L')
			{
				road_yaw[index_six_CCS * 2] = rmin;
			}

			else if (path_six.pathtype[index_six_CCS] == 'R')
			{
				road_yaw[index_six_CCS * 2] = -rmin;
			}
		}

		road_yaw[1] = path_six.pathlength[0] * rmin;
		road_yaw[3] = path_six.pathlength[1] * rmin;
		road_yaw[5] = path_six.pathlength[2] * rmin;
	...
	/////////////////////////////////////////////////////////////////////////////////////
	double delta_six1 = 0;
	if (fabs(road_yaw[0]) > 100000 || fabs(road_yaw[0]) < 5.8)
	{
		delta_six1 = 0;
	}
	else
	...
	double road_yaw_siximag[8] = {0, 0, 0, 0, 0, 0, 0, 0};
	double road_yaw_siximagthree[8] = {0, 0, 0, 0, 0, 0, 0, 0};
	...
	for (int index_D_six = 5; index_D_six > 0; index_D_six--)
	{
		double g_x = 0;
		double g_y = 0;
		double g_th = 0;
		...
		roadsecond_six = road_find_three(g_px, g_py, g_pth, x1, y1, ph, R_V, road_yaw_siximagthree);
		if (roadsecond_six == 0)
		{
			...
			roadsecond_six = RoadCollision(Start_imag, End_imag, road_yaw_siximag);
...
char road_find_seven(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_yaw[8], double road_yaw2[8])
{
	...
	RsPath path_seven;
	double x1_seven = x1 / rmin;
	double y1_seven = y1 / rmin;
	int ErrorCode = 100;
	int theta_car = 0;
	if (fusion.ParkInMode == 1)
	{
	...
	LOG_WARNING("Seven_theta_car=%d", theta_car);
	fflush(bydapa::common::Log::GetInstance()->fileptr());
	ErrorCode = CCS_seven(x1_seven, y1_seven, ph, path_seven, theta_car);
	if (ErrorCode == 0)
	{
		int num = path_seven.pathtype.size();
		for (int index_seven_CCS = 0; index_seven_CCS < num; index_seven_CCS++)
		{
			if (path_seven.pathtype[index_seven_CCS] == 'S')
			{
				road_yaw[index_seven_CCS * 2] = 1000000;
			}

			else if (path_seven.pathtype[index_seven_CCS] == 'L')
			{
				road_yaw[index_seven_CCS * 2] = rmin;
			}

			else if (path_seven.pathtype[index_seven_CCS] == 'R')
			{
				road_yaw[index_seven_CCS * 2] = -rmin;
			}
		}

		road_yaw[1] = path_seven.pathlength[0] * rmin;
		road_yaw[3] = path_seven.pathlength[1] * rmin;
		road_yaw[5] = path_seven.pathlength[2] * rmin;
		path_seven.lenth = path_seven.lenth * rmin;
		if (fabs(road_yaw[5]) > 10)
		{
			return 2;
		}
	}
	...
	End_imag[1] = y_aim;
	End_imag[2] = theta_aim;
	roadsecond_seven = RoadCollision(Start_imag, End_imag, road_yaw);

	if (roadsecond_seven == 0)
	{
		return 0;
	}
	else
	{
		...
		for (int index_D_seven = D_seven; index_D_seven > 0; index_D_seven--)
		{
			...
			double x1_seven_six = x1_seven / rmin;
			double y1_seven_six = y1_seven / rmin;

			double road_yaw_sevenimag[8] = {0, 0, 0, 0, 0, 0, 0, 0};
			ErrorCode = CCS_six(x1_seven_six, y1_seven_six, ph_seven, path_seven, theta_car);
			...
			if (ErrorCode == 0)
			{
				...
				for (int index_seven_CCS = 0; index_seven_CCS < num; index_seven_CCS++)
				{
					...
					{
						road_yaw_sevenimag[(index_seven_CCS + 1) * 2] = rmin;
					}
				...
				road_yaw_sevenimag[3] = path_seven.pathlength[0] * rmin;
				road_yaw_sevenimag[5] = path_seven.pathlength[1] * rmin;
				road_yaw_sevenimag[7] = path_seven.pathlength[2] * rmin;
				path_seven.lenth = path_seven.lenth * rmin;
			}
			roadsecond_seven = RoadCollision(Start_imag, End_imag, road_yaw_sevenimag);
			if (roadsecond_seven == 0)
			{
...
int dynamic_vertical(double Start1[3], double End1[3])
{
	pathpoint.clear();
	...
	LOG_WARNING("theta_car_dynamic=%d", theta_car_dynamic);
	fflush(bydapa::common::Log::GetInstance()->fileptr());
	// C-
	int roadfirst = 1;
	roadfirst = road_find_first(Start[0], Start[1], Start[2], End[0], End[1], End[2], 0.05, R_V, 0.3, road_yaw);
	if (roadfirst == 0)
	{
		roadfirst = RoadCollision(Start, End, road_yaw);
		if (roadfirst == 0)
		{
			LOG_WARNING("roadfirst=%d", roadfirst);
			// fflush(bydapa::common::Log::GetInstance()->fileptr());
			getroadpath(Start, road_yaw);
			return 1;
		}
	}
	// C-S-
	int roadsecond = 1;
	roadsecond = road_find_three_cs(Start[0], Start[1], Start[2] * 180 / pi, End[0], End[1], End[2] * 180 / pi, R_V, 0.03, road_yaw);
	if (roadsecond == 0)
	{
		roadsecond = RoadCollision(Start, End, road_yaw);
		if (roadsecond == 0)
		{
	...
	{
		int roadthird = 1;
		roadthird = road_find_two(Start[0], Start[1], Start[2], End[0], End[1], End[2], R_V, road_yaw);
		if (roadthird == 0)
		{
			roadthird = RoadCollision(Start, End, road_yaw);
			if (roadthird == 0)
			{
	...
	// C+S-
	int roadfourth = 1;
	roadfourth = road_find_four(Start[0], Start[1], Start[2] * 180 / pi, End[0], End[1], End[2] * 180 / pi, R_V, 0, road_yaw);
	if (roadfourth == 0)
	{
		roadfourth = RoadCollision(Start, End, road_yaw);
		if (roadfourth == 0)
		{
	...
	roadfifth = road_find_five(Start[0], Start[1], Start[2], End[0], End[1], End[2], R_V, road_yaw, road_yaw2);
	if (roadfifth == 0)
	{
		roadfifth_Collision = RoadCollision(Start, End, road_yaw);
	...
	{
		roadfifth_Collision = RoadCollision(Start, End, road_yaw2);
		if (roadfifth_Collision == 0)
		{
			LOG_WARNING("roadfifth2=%d", roadfifth);
			// fflush(bydapa::common::Log::GetInstance()->fileptr());
			getroadpath(Start, road_yaw2);
			return 2;
		}
		else
		{
	...
	if (3 - Gears * 2 == -1 * theta_car_dynamic)
	{
		...
		roadsixth = road_find_six(Start[0], Start[1], Start[2], End[0], End[1], End[2], R_V, road_yaw, road_yaw2);
		if (roadsixth == 0)
		{
			roadsixth_Collision = RoadCollision(Start, End, road_yaw);
		...
		{
			roadsixth_Collision = RoadCollision(Start, End, road_yaw2);
			if (roadsixth_Collision == 0)
			{
	...
	{
		int roadseventh = 1;
		int roadseventh_Collision = 100;
		roadseventh = road_find_seven(Start[0], Start[1], Start[2], End[0], End[1], End[2], R_V, road_yaw, road_yaw2);
		if (roadseventh == 0)
		{
			roadseventh_Collision = RoadCollision(Start, End, road_yaw);
			if (roadseventh_Collision == 0)
			{
				LOG_WARNING("roadseventh1=%d", roadseventh);
				// fflush(bydapa::common::Log::GetInstance()->fileptr());
				getroadpath(Start, road_yaw);
				return 2;
			}
		...
		{
			roadseventh_Collision = RoadCollision(Start, End, road_yaw2);
			if (roadseventh_Collision == 0)
			{
				LOG_WARNING("roadseventh2=%d", roadseventh);
				// fflush(bydapa::common::Log::GetInstance()->fileptr());
				getroadpath(Start, road_yaw2);
				return 2;
			}
