...
//RsPath FindRSPathCCS(double x, double y, double phi);
...
// void PopNode(std::vector<Node> *nodes) // 选出代价最小的OPEN节点,返回最小节点的I值
// {
// 	double mincost = 1000; // 如果泊车的总路程大于1000米就不认为是路
// 	int i = 0, idx = 0;
// 	for (i = 0; i < nodes->size(); i++)
// 	{
// 		if ((*nodes)[i].fcost < mincost) //  ???????????????????????????????????小于还是小于等于
// 		{
// 			mincost = (*nodes)[i].fcost;
// 			idx = i;
// 		}
// 	}
// 	g_tnode = (*nodes)[idx];
...
void VehicleDynamic(double x, double y, double theta, double D, double delta) //根据当前位姿和输入, 计算下一位置的位姿
{
	...
	mid_R = vehicle_parameters.WB / (tan(delta)+0.00001);
...
void Update_one(Node wknode, double End[3]) //扩展点 ，并记录点，扩展close、OPEen集合
{
	...
	for (ilr = 0; ilr < find_steer_degree.size(); ilr++)
	{
		...
		str_idx = "xy_yaw"+std::to_string(g_tnode.xidx)+"_"+std::to_string(g_tnode.yidx)+"_"+std::to_string(g_tnode.yawidx);
...
{	
	...
	for (int i = 0; i < 250; i++) {
        for (int j = 0; j < 250; j++) {
            std::cout << voronoiDiagram.getDistance(j, i) << " ";
        }
        std::cout << std::endl;
    }
    ...
    for (int i = 0; i < 250; i++) {
        for (int j = 0; j < 250; j++) {
			float obsDst = voronoiDiagram.getDistance(i, j);
			int colorValue = static_cast<int>(obsDst);
            cv::Vec3b color(255 - colorValue, 0, 128);
            image.at<cv::Vec3b>(i, 249-j) = color;

			if (obstmap[i*250+j].Status == 0)
			{	
				// float x = voronoiDiagram.getDistance(j, i);
				int colorValue = 0;
				cv::Vec3b color(colorValue, 255, 128);
				image.at<cv::Vec3b>(j, 249-i) = color;
			}
			// else
			// {
			// 	int colorValue = 128;
