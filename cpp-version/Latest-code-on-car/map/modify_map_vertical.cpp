...
{
	void ChangeMapPoint(double* vir_x, double* vir_y, int vir_index, int change_flag)
	{
		double x, y;
	...
	void GenerateVirtualPoint()
	{
		...
		int vir_index = 510 / 3;
		double vir_point_x[170];
		double vir_point_y[170];
		...
		if ((ParkingSpaceFlag == 1) && (dist_P01_ > 0.15) && (dist_P01_ < 1.0))
		{
			...
			for (int i = 0; i < vir_index; i++)
			{
				vir_point_x[i] = P_virx - i * 0.03;
				vir_point_y[i] = P_viry;
			}
	...
	void ClearParkSidePoint()
	{
		int index_x = 24 / 3;
		int index_y = 540 / 3;
		double vir_point_x[1440];
		double vir_point_y[1440];
		int vir_index = index_x * index_y;
		double x, y, phi, map_x, map_y;

		CoordinateTrans(x, y, phi, P0x, P0y, .0, end_to_fus[0], end_to_fus[1], end_to_fus[2]);
		//y = y / (fabs(y)); // 1.0m ~ 1.24m
		//std::cout << "prk_bias = " << prk_bias << std::endl;
		y = y + prk_bias;
		for (int i = 0; i < index_x; i++)
		{
			...
			{
				map_x = x + 0.5 - j * 0.03;
				vir_point_x[i * index_y + j] = map_x;
				vir_point_y[i * index_y + j] = map_y;
			}
		}
		ChangeMapPoint(vir_point_x, vir_point_y, vir_index, 1);
		CoordinateTrans(x, y, phi, P3x, P3y, .0, end_to_fus[0], end_to_fus[1], end_to_fus[2]);
		//y = y / (fabs(y));// 1.0m ~ 1.24m
		y = y + prk_bias;
		for (int i = 0; i < index_x; i++)
	...
	void CalcParkFrontDist(double &dist_to_p03)
	{
		...
		for (int i = 0 ; i < row_max; i++)
		{
			...
			{
				int row=(i-row_max)*ParkingSpaceFlag;
				int col = -j;
	...
	void CalcParkSideDist(double& d01, double& d23, const int &status)
	{
		...
		double m_l = .0, min_l_P01 = 1000000, min_l_P23 = 1000000;
	...
	void CalcParkTowardDist(double &toward_dist)
	{
		...
		{
			x_curr=x0;
			y_curr=y0 + space_dist;
		}
		else if (ParkingSpaceFlag == -1)
		{
			x_curr=x3;
			y_curr=y3 - space_dist;
		}
		f_index = GetMapPointIndex(x_curr, y_curr);
			...
			for (int j = 0; j < col_max; j++)
			{
				...
				if (index > 1 && index < pathfind_parameters.MAX_IDX && obstmap[index - 1].Status == 0)
				{
					...
					if(obs_count == obs_count_max)
					{
						...
						if (line_k == 0)
						{
							dist = abs((double)m_y - (y_curr - ParkingSpaceFlag * space_dist));
						}
						else
						{									
							double line_b = -line_k * x_curr + (y_curr - ParkingSpaceFlag * space_dist);
							dist = abs(line_k * (double)m_x - (double)m_y + line_b);
							dist = dist / (sqrt(1 + line_k * line_k));
						}
    ...
    void ModifyMapVertical()
    {
        ...
        {
            if (PlanBackFirst < 3 && PlanForwardFirst < 5) //2023.03.09
            {
                // CalcParkSideDist(dist_P01, dist_P23);
                // CalcParkFrontDist(dist_P03);
				// CalcParkTowardDist(dist_toward_P03);
        ...
        {
            if (PlanBackFirst < 3) //2023.03.09
            {
                // CalcParkSideDist(dist_P01, dist_P23);
                // CalcParkFrontDist(dist_P03);
				// CalcParkTowardDist(dist_toward_P03);
				//std::cout << "toward_dist = " << dist_toward_P03 << std::endl;
            }
        }
    }
}
