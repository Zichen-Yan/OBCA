...
namespace byd_apa_plan
{
	...
	int  geometry_Level_out(double Start1[3], double End1[3]);
	/////////////////////////////////////////////////////////////////////
	void getroadpath_Level_all_out(double Start[3], double road_roll[28], double D, double R)
	{
		path_point mid_L;
	...
	bool VehicleCollisionGridLevel_out(double cpx, double cpy, double cph, int Rect_)
	{
		...
		if (Rect_ != 0)
		{
			for (unsigned int i = 0; i < Rect_x.size(); i++)
			{
				...
				else if ((Rect_ == 2) && (obstmap[idx_xy].Status == 6))
				{
					isCollision = true;
					return isCollision;
				}
		...
		{
			for (unsigned int i = 0; i < Rect_x0.size(); i++)
			{
