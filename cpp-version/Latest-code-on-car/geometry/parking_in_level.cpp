...
namespace byd_apa_plan
{
			...
			else // 库外求解
			{
				...
				if (choice_cc == 0)
				{
					...
					if ((R_C <= 0) && (road_roll[1] <= 0) && (road_roll[3] <= 0) && (road_roll[5] <= 0))
					{
						choice_cc = 1;
						/// 24.1.11 /// 
						Path_2 = 1; // 请求规划不再需要调平车辆时， 开启动态规划
						end_init[0] = end_to_fus[0];
						end_init[1] = end_to_fus[1];
						end_init[2] = end_to_fus[2];
						/// 24.1.11 ///
					}
				}
				else
	...
	int  ParkingInLevel(double Start1[3], double End1[3])
	{
		...
		if (roadfirst == 0)
		{
			...
			if (roadfirst == 0)
			{
				if (Start[0] > End[0] + 0.3 ) //&& fabs(Start[1] - End[1]) > 0.3 
				{
					Path_2 = 1; // 以C求解时缺少动态规划停止条件？
				    choice_cc = 1; 
				}
		...
		if (roadthird == 0) 
		{
			...
			if (roadthird == 0)
			{
				if (Start[0] > End[0] + 0.3 && fabs(Start[1] - End[1]) > 0.3) // 符合区间内的采取动态规划
				{
					Path_2 = 1; // 
				    choice_cc = 1; 
				}
