        ...
        while (NotFinishTable.norm() > 0)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                case 0:
                    ...
                    // CheckCollisionUsingGlobalPosition(point1, &res1);
                    // CheckCollisionUsingGlobalPosition(point2, &res2);
                    // if (res1==true || res2==true)
                    // {
                    //     std::cout << "init point is infeasible:" << std::endl;
                               ...
                               const Eigen::Vector2d p2, bool *res, double checkl)
{
    for (double dl = 0.0; dl < (p2 - p1).norm(); dl += checkl)
    {
        ...
        CheckCollisionUsingGlobalPosition(pos, res);
        if (*res)
        {   // collision
            return;
        }
    }
    CheckCollisionUsingGlobalPosition(p2, res);
}
...
void CheckCollisionUsingGlobalPosition(const Vec2f &p_w, bool *res)
{
    ...
    // }
    // *res = false;
    int xidx = ceil((pathfind_parameters.MAXX - p_w[0]) / pathfind_parameters.XY_GRID_RESOLUTION);
    int yidx = ceil((pathfind_parameters.MAXY - p_w[1]) / pathfind_parameters.XY_GRID_RESOLUTION);
    if (xidx == 0)
    {
        xidx = 1;
    }
