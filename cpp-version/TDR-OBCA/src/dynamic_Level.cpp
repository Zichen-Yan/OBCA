...
char road_find_seven1_Level(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit, double road_yaw[6])
{
	...
	{
		if (fabs(theta_ang_seven) < 1 * M_PI / 180) //判断车头方向是否超调，如果超调证明空间足够，请求A*
		{
...
char road_find_seven2_Level(double x_now, double y_now, double theta_now, double x_aim, double y_aim, double theta_aim, double car_r_limit)
{
	...
	if (theta_ang_seven >= 0)
	{
		...
		else if (theta_ang_seven > 60 * M_PI / 180)
		{
			return 1;        // 夹角大于60°，失败
		}
...
int  dynamic_Level(double Start1[3], double End1[3])
{
	...
	//C-S-
	int roadsecond = 1;
	roadsecond = road_find_three_cs_Level(Start[0], Start[1], Start[2] * 180 / M_PI, End[0], End[1], End[2] * 180 / M_PI, R_L, 0.0, road_yaw);
	if (roadsecond == 0)
	{
	...
	//C+S- 最小半径
	int roadfourth = 1;
	roadfourth = road_find_four_Level(Start[0], Start[1], Start[2] * 180 / M_PI, End[0], End[1], End[2] * 180 / M_PI, R_L, 0.05, road_yaw);
	if (roadfourth == 0)
	{
