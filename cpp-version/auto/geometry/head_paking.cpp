...
namespace byd_apa_plan{
	...
	int CalcCmCpSpMulti(const double *start, const double *end)
	{
		...
		RsPath rs_path;
		double road_yaw[6] = {.0};
		int parking_mode = (fusion.ParkInMode == 1) ? -1 : 1;
	...
	bool CalcCpCpRadius(const double &x, const double &y, const double &phi, double &min_r)
	{
		...
		if(!(delta < 0 || (a == 0 && b == 0)))
		{
			if(a == 0)
			{
				min_r = (fabs(c * 0.000001) >= fabs(b)) ? 1e6 : (-c/b);
				return true;
			}
			else{
				r_1 = (fabs((-b + pow(delta, 0.5)) * 0.000002) >= fabs(a)) ? 1e6 : ((-b + pow(delta, 0.5)) / 2 / a);
				r_2 = (fabs((-b - pow(delta, 0.5)) * 0.000002) >= fabs(a)) ? 1e6 : ((-b - pow(delta, 0.5)) / 2 / a);
	...
	bool CpCp(const double &x, const double &y, const double &phi, const double& curr_min,double* road_yaw)
	{
		...
		if(!(delta < 0 || (a == 0 && b == 0)))
		{
			if(a == 0)
			{
				r_1 = (fabs(c * 0.000001) >= fabs(b)) ? 1e6 : (-c/b);
	...
	bool CalcSafeRadiusCpCp(const double* start, const double* end, const double &r_safe)
	{
		...
		if(CalcCpCpRadius(x, y, phi, min_r) && fabs(min_r - .0) > 1e-5)
		{
			...
			if(fabs(sin_res) - 1.0 < .0)
			{
				...
				if((fabs(y_error) - max_y_error_completed < 1e-5) && (t >= 0))
				{
	...
	bool CalcHeadPathMulti(double* start,double* end)
	{
		...
		if(Gears == 2 && (PlanBackwardFirst >= 2))
		{
			flag_backward = false; 
		}
		while (index < 5)
	...
	bool SimplifyPath(double* start, double *start_p,int &critical_size, const double &r_curr, const double &theta)
	{
		...
		std::vector<PathPoint> pathpoint_cp(pathpoint.begin(),pathpoint.end());
		//test Cm simplify
		...
		//2024.03.25 add CmCm simplify path
		double road_yaw[6] = {.0};
		if(Calc_CmCm(start[0], start[1], start[2], end[0], end[1], end[2], rmin_vertical, road_yaw) == 0)
		{
			if (GetOrCheckPathPoints(start, road_yaw, 3, 1))
			{
				pathpoint.clear();
	...
	bool CalcHeadPathDirectly(const double* start, const double* end)
	{
		...
		//CpCp
		if(fabs(start[2]) < 2*pi/18)  //2024.03.25 10 -> 20
		{	
			...
			if (CalcCriticalPosition(critical_pos) && CalcPathToCriticalPostion(start_p,critical_pos,r_curr,theta))
			{
				...
				while(1)
				{
					...
					if(CalcHeadPathMulti(critical_pos,end))
					{
						...
						if(true && SimplifyPath(start,start_p,critical_size, r_curr, theta))
						{
							PlanBackwardFirst ++;
						}
						PlanBackwardFirst++;
