...
double plan_x[300] = {.0};
double plan_y[300] = {.0};
double plan_yaw[300] = {.0};
double plan_curvature[300] = {.0};
int plan_rajectory_direction[300] = {0};
double path_length = 0;
int change_gear_times = 0;
std::string plan_time_show = "";
const std::string plan_all_time_file = "/home/byd2004/Pictures/auto/python/000plan.txt";
Plan plan;

void PlotResult(char *data_name, std::string plan_time_show);
void CalcGridCoordinate(const int &index_, double &m_x, double &m_y);
void read_map_data(char *data_name)
{
    std::ifstream file(data_name);
    std::ifstream file2(data_name);

    std::string s;
    if (!file.is_open())
    {
        std::cout << "wronge file read\n";
    }
    std::string line;
    std::vector<std::string> line_read;
    std::string key;
    unsigned int line_read_all = 0;
    while (std::getline(file, line))
    {
        line_read_all++;
    }
    unsigned int select_plan_times = floor(line_read_all / 333 / 2) + 1;
    select_plan_times = 1;
    std::string line_collect[333] = {};
    unsigned int line_read_0 = 0;
    unsigned int line_read_1 = 0;

    std::string line2;
    while (std::getline(file2, line2))
    {
        line_read_0++;
        if (((select_plan_times - 1) * 333 < line_read_0) && (line_read_0 < select_plan_times * 333 + 1))
        {
            line_collect[line_read_1] = line2;
            line_read_1++;
        }
    }
    ...
    unsigned int line_location = 0;
    for (unsigned int i = 0; i < 333; i++)
    {
        line = line_collect[i];
        line_location++;
        bool read_line_flag = true;
        if (line_location == 2 || line_location == 3 || line_location == 12 || line_location == 14 || line_location == 15 || line_location == 20 || line_location == 29 ||
            ((line_location >= 35) && (line_location <= 334)))
        {
        ...
        if (line_location == 2 || line_location == 3 || line_location == 12 || line_location == 14 || line_location == 15 || line_location == 20 || line_location == 29 || ((line_location >= 35) && (line_location <= 334)))
        {
            for (char &c : line)
            {
                if (c == ',')
    ...
    unsigned int line_i_read = 0;
    for (auto line_i : line_read)
    {
        line_i_read++;
        std::istringstream iss(line_i);
        if (line_i_read == 1)
        {
            iss >> fusion_position[0] >> fusion_position[1] >> fusion_position[2];
        }
        if (line_i_read == 2)
        {
            iss >> nav_pos[0] >> nav_pos[1] >> nav_pos[2];
        }
        if (line_i_read == 3)
        {
            iss >> park[0] >> park[1] >> park[2] >> park[3] >> park[4] >> park[5] >> park[6] >> park[7];
        }
        if (line_i_read == 4)
        {
            iss >> start_to_fus[0] >> start_to_fus[1] >> start_to_fus[2];
        }
        if (line_i_read == 5)
        {
            iss >> end_to_fus[0] >> end_to_fus[1] >> end_to_fus[2];
        }
        if (line_i_read == 6)
        {
            for (int i_map = 0; i_map < 62500; i_map++)
            {
                iss >> obstmap[i_map];
            }
        }
        ...
        if (line_i_read == 7)
        {
            plan_time_show = line_i;
        }

        if (line_i_read >= 8)
        {
            iss >> plan_x[line_i_read - 8] >> plan_y[line_i_read - 8] >> plan_yaw[line_i_read - 8] >> plan_curvature[line_i_read - 8] >> plan_rajectory_direction[line_i_read - 8];
        }
    }
    for (int i = 0; i < 299; i++)
    {
        if (plan_rajectory_direction[i] != plan_rajectory_direction[i + 1] && plan_rajectory_direction[i + 1] != 0)
        {
            change_gear_times++;
        }
    }
    PlotResult(data_name, plan_time_show);
}
...
void PlotResult(char *data_name, std::string string_input)
{
    // plt::figure();
    ...
    // park
    double p_x[4] = {park[0], park[2], park[4], park[6]};
    double p_y[4] = {park[1], park[3], park[5], park[7]};
    ...
    for (int i = 0; i < 300; i++)
    {
        if (!(plan_x[i] == 0 && plan_y[i] == 0 && plan_yaw[i] == 0))
        {
            x.push_back(plan_x[i] * 0.01);
            y.push_back(plan_y[i] * 0.01);
            yaw.push_back(plan_yaw[i] * pi / 180);
        }
    }
    // map
    for (int idx = 1; idx <= 62500; idx++)
    {
        ...
        else if (obstmap[idx - 1] == 3)
        {
            v_x.push_back(x_c);
            v_y.push_back(y_c);
        }
        else if (obstmap[idx - 1] == 6)
        {
            u_x.push_back(x_c);
            u_y.push_back(y_c);
        }
    ...
    for (int i = 0; i < 300; i++)
    {
        if (plan_rajectory_direction[i] == 0)
        {
            plot_size = i;
            break;
        }
    ...
    path_length = (plot_size) * 10 * 0.01;
    for (int plot_num = 0; plot_num <= plot_size - 1; plot_num += 1)
    {
        std::vector<double> car_x_mid;
        std::vector<double> car_y_mid;
        for (int i = 0; i < Rect_index; i++)
        {
            double x = Rect_x[i] * cos(plan_yaw[plot_num] * pi / 180) - Rect_y[i] * sin(plan_yaw[plot_num] * pi / 180) + plan_x[plot_num] * 0.01;
            double y = Rect_x[i] * sin(plan_yaw[plot_num] * pi / 180) + Rect_y[i] * cos(plan_yaw[plot_num] * pi / 180) + plan_y[plot_num] * 0.01;
            car_x_mid.push_back(x);
            car_y_mid.push_back(y);
        }
        plt::plot(car_x_mid, car_y_mid, "g-");
    }
    //
    ...
    plot_name = data_name;
    change_gear_times--;
    std::string pathlength_show_no_m = "path_length = " + std::to_string(path_length);
    std::string pathlength_show = "path_length = " + std::to_string(path_length) + "m";
    std::string change_gear_times_show = "change_gear_times =" + std::to_string(change_gear_times);
    // std::string all_time_show = "all_time = " + std::to_string(all_time) + "s";

    // annotation
    plt::text(-4, 14, pathlength_show);
    plt::text(-4, 13, change_gear_times_show);

    plt::title(string_input);
    plt::grid(true);
    plt::xlabel(plot_name + " x [m]");
    plt::ylabel("y [m]");
    // plt::show();
    plt::save("test.png");
    ...
    outfile.open(plan_all_time_file, std::ios::app);
    if (!outfile) {
        std::cerr << "无法打开文件 " << std::endl;
    }
    outfile << plan_time_show << " "<< pathlength_show_no_m << " "<<change_gear_times_show << "\n";
    outfile.close();
}
