...
private:
	double width_ = vehicle_parameters.W+0.4;	   // 1.90;
	double length_ = vehicle_parameters.LF+vehicle_parameters.LB+0.4;	   // 4.88;
	double wheel_base_ = vehicle_parameters.WB; // 2.85;
	double front_suspension_ = vehicle_parameters.LF-vehicle_parameters.WB;
	double rear_suspension_ = vehicle_parameters.LB;
	double max_steering_angle_ = vehicle_parameters.MAX_STEER/M_PI*180; // 45.0;
...
bool CheckIfCollisionUsingLine(const Eigen::Vector2d p1, const Eigen::Vector2d p2, bool *res, double checkl);
