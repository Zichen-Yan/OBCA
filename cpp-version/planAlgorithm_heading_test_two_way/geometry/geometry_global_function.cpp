...
namespace byd_apa_plan
{
	bool VehicleCollisionGrid(double cpx, double cpy, double cph)
	{
		...
		{
			rect_x = Rect_x20[i] * cosphi + Rect_y20[i] * sinphi + cpx_;
			rect_y = Rect_y20[i] * cosphi - Rect_x20[i] * sinphi + cpy_;
			//栅格化
	...
	bool GetOrCheckPathPoints(const double* start, const double* road_yaw, const int& i_coll_max, const int &collision_check_flag)
	{
		...
		for(int i_coll = 0; i_coll < i_coll_max; i_coll++)
		{
			...
			delta = (fabs(road_yaw[2*i_coll]) >= (1e6-1) || fabs(road_yaw[2*i_coll]) < rmin_vertical*0.5) ? .0 : atan(vehicle_parameters.WB / road_yaw[2*i_coll]);
	...
	void CoordinateTransToPrk()
	{
		...
		{
			if(app.APA_Park_Function == 4 || app.APA_Park_Function == 5)
			{
			...
			else
			{
				if (end_to_fus[2] > 0)
				{
					ParkingSpaceFlag = (fusion.ParkInMode == 1) ? -1 : 1;
		...
		else
		{
			...
			else
			{
				if (index_request == 1)
				{
					if (start_to_prk[1] > 1) // 右泊
					{
						right_left = 1;
					}
					else if (start_to_prk[1] < -1) // 左泊
					{
						right_left = -1;
					}
				}
	...
	std::vector<PathPoint> PathPointsCoordinateTrans(const std::vector<PathPoint>& pathpoints_, double end_to_fus_point[3])
	{
