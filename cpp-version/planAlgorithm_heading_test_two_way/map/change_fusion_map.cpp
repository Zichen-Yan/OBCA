...
namespace byd_apa_plan
{
	...
	void Uss_map()
	{
		...
		if ((control.ObsUssInfo == 0u) || (control.ObsUssInfo == 1u) || (control.ObsUssInfo == 2u) || (control.ObsUssInfo == 3u) || (control.ObsUssInfo == 6u) || (control.ObsUssInfo == 7u) || (control.ObsUssInfo == 8u) || (control.ObsUssInfo == 9u))
		{
			...
			{
				P0x_imag = P0x_imag + 1 * cos(end_to_prk[2]);
				P0y_imag = P0y_imag + 1 * sin(end_to_prk[2]);
				P3x_imag = P3x_imag + 1 * cos(end_to_prk[2]);
				P3y_imag = P3y_imag + 1 * sin(end_to_prk[2]);
			}
	...
	void CalcVerticalOutVirtualPoint()
	{
		if(index_request == 1 && plan_request == 1u)
		{
			double x = .0, y = .0, phi = .0;
			double fus_theta = mod2pi(fusion.position.Heading * pi / 180);
			double op_theta = mod2pi(currentParkInfo.Angle * pi / 180);
			CoordinateTrans(x, y, phi, currentParkInfo.CurrentPark.P0_X, currentParkInfo.CurrentPark.P0_Y, 0.0, fusion.position.X, fusion.position.Y, fus_theta);
			double P0x_op = x / 100.0;
			double P0y_op = y / 100.0;
			//CoordinateTrans(x, y, phi, currentParkInfo.CurrentPark.P1_X, currentParkInfo.CurrentPark.P1_Y, 0.0, fusion.position.X, fusion.position.Y, fus_theta);
			//double P1x_op = x / 100.0;
			//double P1y_op = y / 100.0;
			//CoordinateTrans(x, y, phi, currentParkInfo.CurrentPark.P2_X, currentParkInfo.CurrentPark.P2_Y, 0.0, fusion.position.X, fusion.position.Y, fus_theta);
			//double P2x_op = x / 100.0;
			//double P2y_op = y / 100.0;
			CoordinateTrans(x, y, phi, currentParkInfo.CurrentPark.P3_X, currentParkInfo.CurrentPark.P3_Y, 0.0, fusion.position.X, fusion.position.Y, fus_theta);
			double P3x_op = x / 100.0;
			//double P3y_op = y / 100.0;
			if (app.APA_Park_Function == 4)
			{
				double x_P0 = P0x_op + (0.6 + vehicle_parameters.W/2) * cos(op_theta - pi/2);
				double y_P0 = P0y_op + (0.6 + vehicle_parameters.W/2) * sin(op_theta - pi/2);
				double y_diff = right_left * (rmin_vertical-rmin_vertical * cos(end_to_fus[2]))+Start_out[1];
				double x_diff = 0;
				if (((fabs(end_to_fus[2])>85*pi/180) && (fabs(end_to_fus[2])<95*pi/180)) || (fabs(end_to_fus[2])<5*pi/180) || (fabs(end_to_fus[2])>175*pi/180))
				{
					x_diff = (P0x_op+P3x_op)/2;
				}
				else
				{
					x_diff = (y_diff - y_P0) / tan(end_to_fus[2]) + x_P0;
				}			 
				dist_out_min = x_diff - fabs(rmin_vertical * sin(end_to_fus[2])) - Start_out[0];
				int D_m = dist_out_min * 10;
				dist_out_min = D_m * 0.1 + 0.1;
				if (dist_out_min < 2.2) // 大于2.2m证明一把能泊出并距离0.6m
				{
					dist_out_min = 2.2;
				}

				double map_out_detection = dist_out_min + 4 + 0.1;
				std::pair<double, double> vir_point;
				for (int index_x = 0; index_x < 30; index_x++)
				{
					map_out_detection = map_out_detection - 0.1;
					for (int index_y = 0; index_y < 40; index_y++)
					{
						vir_point.first = map_out_detection + index_y * 0.1 * cos(op_theta);
						vir_point.second = 1.2 + index_y * 0.1 * sin(op_theta);
						park_out_head_left.push_back(vir_point);
						vir_point.first = map_out_detection - index_y * 0.1 * cos(op_theta);
						vir_point.second = 1-1.2 - index_y * 0.1 * sin(op_theta);
						park_out_head_right.push_back(vir_point);
					}
				}
			}
			else if (app.APA_Park_Function == 5)
			{
				dist_right_out = P0x_op - Start_out[0] + 1.1;
				dist_left_out = P3x_op - Start_out[0] + 1.1;
				dist_out_min = dist_right_out>dist_left_out?dist_right_out:dist_left_out;
				int D_m = (dist_out_min - 1.1) * 10;
				dist_out_min = D_m * 0.1;
				double map_out_detection = dist_out_min - 3;
				std::pair<double, double> vir_point;
				for (int index_x = 0; index_x < 30; index_x++)
				{
					map_out_detection = map_out_detection + 0.1;
					for (int index_y = 0; index_y < 40; index_y++)
					{
						vir_point.first = map_out_detection + index_y * 0.1 * cos(op_theta);
						vir_point.second = 1.2 + index_y * 0.1 * sin(op_theta);
						park_out_rear_left.push_back(vir_point);
						vir_point.first = map_out_detection - index_y * 0.1 * cos(op_theta);
						vir_point.second = 1-1.2 - index_y * 0.1 * sin(op_theta);
						park_out_rear_right.push_back(vir_point);
					}
				}
			}
		}
	...
	void ChangeFusionMap()
	{
		...
		if (fusion.parkingSpaceInfo.ParkingSpaceType == 1 || fusion.parkingSpaceInfo.ParkingSpaceType == 3)
		{
			...
			else if(app.APA_Park_Function == 4 || app.APA_Park_Function == 5)
			{
				CalcVerticalOutVirtualPoint();
			}
		}
	}
