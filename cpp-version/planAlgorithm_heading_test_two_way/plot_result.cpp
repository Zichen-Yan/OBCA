...
namespace byd_apa_plan
{
    ...
    void PlotResult()
    {
        ...
        for (unsigned int i = 0; i < Rect_x20_sparse.size(); i++)
        {
            double x = Rect_x20_sparse[i] * cos(start_to_fus[2]) - Rect_y20_sparse[i] * sin(start_to_fus[2]) + start_to_fus[0];
            double y = Rect_x20_sparse[i] * sin(start_to_fus[2]) + Rect_y20_sparse[i] * cos(start_to_fus[2]) + start_to_fus[1];
            car_x.push_back(x);
            car_y.push_back(y);
        }
        //
        ...
        for (int plot_num = 0; plot_num < plot_size - 1; plot_num += 2)
        {
            ...
            for (unsigned int i = 0; i < Rect_x20_sparse.size(); i++)
            {
                double x = Rect_x20_sparse[i] * cos(plan.coordinate[plot_num].Yaw * pi / 180) - Rect_y20_sparse[i] * sin(plan.coordinate[plot_num].Yaw * pi / 180) + plan.coordinate[plot_num].X * 0.01;
                double y = Rect_x20_sparse[i] * sin(plan.coordinate[plot_num].Yaw * pi / 180) + Rect_y20_sparse[i] * cos(plan.coordinate[plot_num].Yaw * pi / 180) + plan.coordinate[plot_num].Y * 0.01;
        ...
        for (unsigned int i = 0; i < Rect_x20_sparse.size(); i++)
        {
            double x = Rect_x20_sparse[i] * cos(end_to_fus[2]) - Rect_y20_sparse[i] * sin(end_to_fus[2]) + end_to_fus[0];
            double y = Rect_x20_sparse[i] * sin(end_to_fus[2]) + Rect_y20_sparse[i] * cos(end_to_fus[2]) + end_to_fus[1];
