...
namespace byd_apa_plan
{
    ...
    bool ReadParkConfig(const nlohmann::json &j)
    {
        ...
        max_y_error_completed = j.at("park").at("vertical").at("max_y_error_completed").get<double>();
    ...
    bool ReadGeometryConfig(const nlohmann::json &j)
    {
        ...
        depth_no_obstacle = j.at("geometry").at("vertical").at("rear_parking").at("depth_no_obstacle").get<double>();
	    length_of_straight = j.at("geometry").at("vertical").at("rear_parking").at("length_of_straight").get<double>();
        delta_r = j.at("geometry").at("vertical").at("rear_parking").at("delta_r").get<double>();
        //parking out
        default_dist_head_out = j.at("geometry").at("vertical").at("parking_out").at("default_dist_head_out").get<double>();
        default_dist_out_min = j.at("geometry").at("vertical").at("parking_out").at("default_dist_out_min").get<double>();
        default_dist_rear_out = j.at("geometry").at("vertical").at("parking_out").at("default_dist_rear_out").get<double>();
        //mapper
        col_max_front = j.at("geometry").at("vertical").at("mapper").at("col_max_front").get<int>();
        row_max_front = j.at("geometry").at("vertical").at("mapper").at("row_max_front").get<int>();
        col_max_toward = j.at("geometry").at("vertical").at("mapper").at("col_max_toward").get<int>();
        row_max_toward = j.at("geometry").at("vertical").at("mapper").at("row_max_toward").get<int>();
        toward_space_start = j.at("geometry").at("vertical").at("mapper").at("toward_space_start").get<double>();
        obs_count_max = j.at("geometry").at("vertical").at("mapper").at("obs_count_max").get<int>();
        virtual_obs_length = j.at("geometry").at("vertical").at("mapper").at("virtual_obs_length").get<double>();
        //head
    ...
    bool ReadCollisionModelConfig(const nlohmann::json &j)
    {
        ...
        key_x = {"collisionModel","model_20cm_sparse_x",};
        key_y = {"collisionModel","model_20cm_sparse_y",};
        if(!ReadArrayFromConfig(Rect_x20_sparse, Rect_y20_sparse, j, key_x, key_y))
        {
            //std::cout << "false\n";
            return false;
        }
        ...
        if(!ReadArrayFromConfig(Rect_x20, Rect_y20, j, key_x, key_y))
        {
            //std::cout << "false\n";
            return false;
        }
    ...
    bool ReadFileNameConfig(const std::string &filename, std::string &vehParam_filename)
    {
        ...
        vehParam_filename = j.at("vehParamsFile").get<std::string>();
        if(!vehParam_filename.empty())
        {
            ifile.close();
            return true;
        }
    ...
    bool ReadVehicleParamFromConfig(const std::string &filename)
    {
        std::string veh_param_filename;
        if(ReadFileNameConfig(filename, veh_param_filename))
        {
            std::ifstream ifile(veh_param_filename);
            nlohmann::json j;
            try{
                j = nlohmann::json::parse(ifile);
            }catch (nlohmann::json::parse_error& ex) {
            ...
            vehicle_parameters.W = 0.001 * j.at("vehicle_size").at("width").get<double>();
            vehicle_parameters.WB = 0.001 * j.at("wheelbase").get<double>();
            vehicle_parameters.LF = 0.001 * j.at("shaft_center_to_start").get<double>();
            vehicle_parameters.LB = 0.001 * j.at("shaft_center_to_end").get<double>();
            vehicle_parameters.MAX_STEER = j.at("max_turning_angle").get<double>();
            vehicle_parameters.MIN_CIRCLE = j.at("min_turning_radius").get<double>();
            return true;
        }
        return false;
    }
    ...
    bool ReadConfig(const std::string &vehicle_filename, const std::string &plan_filename)
    {
        return ReadVehicleParamFromConfig(vehicle_filename) && ReadPlanConfig(plan_filename);
    }
