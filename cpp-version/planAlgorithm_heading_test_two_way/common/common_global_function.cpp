...
namespace byd_apa_plan
{
	...
	void GetPrkVetical(double start[3], double b_depth, double park_theta)
	{
		...
		if (b_depth - 0.1 > 0.0)
		{
			double dist = (fusion.ParkInMode == 1) ? -(b_depth - vehicle_parameters.WB - dist_to_block) : (b_depth - dist_to_block);
		...
		{
			double dist = (fusion.ParkInMode == 1) ? -vehicle_parameters.LF : vehicle_parameters.LB ;
	...
	void GetPrkInclined(double start[3], const double& b_depth, const double& park_theta)
	{
		...
		else
		{
			...
			if (dist_02 < dist_13)
			{
				...
				if (D_imgP2P3 >= 5)
				{
					double dist = (fusion.ParkInMode == 1) ?  -vehicle_parameters.LB : vehicle_parameters.LF;
		...
		if ((fabs(delta_theta) > 10 * pi / 180) && (diffSE > 1.44))
		{
			bias_dist = (fusion.ParkInMode == 1) ? -bias_dist : bias_dist;
			end_to_fus[0] = end_to_fus_before[0] + (park_theta / fabs(park_theta)) * bias_dist * sin(park_theta);
			end_to_fus[1] = end_to_fus_before[1] - (park_theta / fabs(park_theta)) * bias_dist * cos(park_theta);
			end_to_fus[2] = end_to_fus_before[2];
			//prk_bias = -(park_theta / fabs(park_theta)) * 0.1;
	...
	{
		bool flag_check_input_data = ((fusion.position.Heading >= 0) && (fusion.position.Heading <= 360)) && ((calculation.nav_heading >= 0) && (calculation.nav_heading <= 360))
								&& ((fusion.parkingSpaceInfo.ParkingSpaceType == 1) || (fusion.parkingSpaceInfo.ParkingSpaceType == 2) || (fusion.parkingSpaceInfo.ParkingSpaceType == 3))
								&& ((fusion.ParkInMode == 0) || (fusion.ParkInMode == 1))
								&& ((app.APA_Park_Function == 1) || (app.APA_Park_Function == 2) || (app.APA_Park_Function == 3) || (app.APA_Park_Function == 4) ||(app.APA_Park_Function == 5));
		if(flag_check_input_data)
		{
			if (control.ObsUssInfo == 20u)
			{
				block_collison_times++;
			...
			double cal_theta = mod2pi(calculation.nav_heading * pi / 180); 
	...
	void IsParkingCompleted()
	{
		...
		if (plan_request == 1u)
		{
			if(app.APA_Park_Function == 4)
			{
				if (right_left != 0)
				{
					if (fabs(start_to_fus[2]-end_to_fus[2]) < 5*pi/180)
					{
						flag_completed = true;
					}
				}
				...
				{
					if ((fabs(start_to_fus[0]-Start_out[0]-dist_head_out) < 1) && (fabs(start_to_fus[2])<2*pi/180))
					{
						flag_completed = true;
					}
				}
			...
			else if(app.APA_Park_Function == 5)
			{
				...
				{
					if ((fabs(start_to_fus[0]-Start_out[0]-dist_rear_out) < 1) && (fabs(start_to_fus[2])<2*pi/180))
					{
						flag_completed = true;
					}
				}
			...
			else
			{
				...
				flag_completed = (fabs(start_to_prk[0]) < Complete_x) && (fabs(start_to_prk[1]) < Complete_y) && (fabs(start_to_prk[2]) < Complete_theta);
			}
	...
	void StateClear()
	{
		...
		if ((app.APAStatus == 0u) || (app.APAStatus == 1u) || (app.APAStatus == 4u) || (app.APAStatus == 5u) || (app.APAStatus == 6u) || (app.APAStatus == 7u))
		{
			...
			choice_cc = 0;
			flag_dynamic_plan_advanced = false;
			FLAG_straight_forward = false;
			FLAG_published_end = false;
			FLAG_forward_once_more = true;
			P_r_x_first = .0;
			P_r_y_first = .0;
			first_path_end.first = 100.0;
			first_path_end.second = 100.0;
			...
			// out 
			flag_out_park = 0;
			dist_out_min = default_dist_out_min;
			dist_head_out = default_dist_head_out;
			dist_rear_out = default_dist_rear_out;
			dist_left_out = .0;
			dist_right_out = .0;
			for(int i = 0; i < 3; i++)
			{
	...
	void PathPointNormalization()
	{
		...
		double theta = .0;
		double curve = .0;
		bool flag_store_end = false;
		for(int idx = 0; idx < pp_size_now; idx++)
		{
			//store first path end
			if(!flag_store_end && idx > 0 && idx < pp_size_now - 1 && PlanBackwardFirst == 0 && PlanForwardFirst == 2)
			{
				if(pathpoint[idx].D > 0 && pathpoint[idx+1].D < 0)
				{
					first_path_end.first = pathpoint[idx].x;
					first_path_end.second = pathpoint[idx].y;
					flag_store_end = true;
				}
			}
			//
