...
namespace byd_apa_plan
{
	...
	RsPath FindRSPathCC(double x, double y, double phi);
	RsPath FindRSPathAll(double x, double y, double phi);
	double TotalCost(Node wknode, double End[3]);
	int inNodes(Node node, std::vector<Node>* nodes);
	void Update(Node wknode, double End[3]);
	void Update_twoway(Node wknode1, Node wknode2, double End[3], double End_two[3]);
	bool CalcNextNode(Node wknode, double D, double delta);
	RsPath AnalysticExpantion(Node wknode, double End[3]);
	RsPath AnalysticExpantion_RS(Node wknode, double End[3]);
	RsPath AnalysticExpantion_CC(Node wknode, double End[3]);
	void getFinalPath(RsPath path, std::string stridx);
	void getFinalPath_onlyRS(RsPath path, PathPoint pt);
	void getFinalPath_onlyAstar(std::string stridx_two);
	...
	bool CalcNextNode(Node wknode, double D, double delta)//根据当前点及距离及方向盘角度计算下一点
	{
		...
		for (i = 0; i < nlist; i++)
		{
			...
			auto tmp = VehicleCollisionGrid_Astar(g_px, g_py, g_pth);
			if (tmp == true)
			{
				return false;
			}
	...
	void Update_twoway(Node wknode1, Node wknode2, double End[3], double End_two[3])
	{
		unsigned int ilr = 0; //左右转
		double delta = -vehicle_parameters.MAX_STEER; //右转最小转角
		double mres = pathfind_parameters.MOTION_RESOLUTION; // 运动分辨率
	...
	RsPath AnalysticExpantion_RS(Node wknode, double End[3]) //参数节点的RS曲线是否发生碰撞
	{
		bool isCollision = false;
		// 将起点转换到原点计算轨迹，变换坐标系了
		double rmin = vehicle_parameters.MIN_CIRCLE;
	...
	RsPath AnalysticExpantion_CC(Node wknode, double End[3]) //参数节点的RS曲线是否发生碰撞
	{
		bool isCollision = false;
		// 将起点转换到原点计算轨迹，变换坐标系了
		double x = End[0] - wknode.x;
		...
		else
		{
			...
			double segs[3] = { path.road_yaw[1] , path.road_yaw[3], path.road_yaw[5] };
			int i = 0;
			double D = 0;
			double delta = 0;
			int idx = 0;
			for (i = 0; i < 3; i++)
			{
				...
				D = (segs[i] > 0) ? pathfind_parameters.MOTION_RESOLUTION : -pathfind_parameters.MOTION_RESOLUTION;
				delta = atan(vehicle_parameters.WB / path.road_yaw[i*2]);
	...
	void getFinalPath_CC(RsPath path, PathPoint pt)
	{
		int i = 1, idx = 0;
		std::vector<Node> nodes;
		double segs[3] = { path.road_yaw[1] , path.road_yaw[3], path.road_yaw[5] };
		...
		for (i = 0; i < 3; i++)
		{
			...
			for (idx = 0; idx < round(fabs(segs[i]) / pathfind_parameters.MOTION_RESOLUTION); idx++) // round()四舍五入
			{
				VehicleDynamic(pt.x, pt.y, pt.th, D, delta); // 根据当前位姿和输入, 计算下一位置的位姿
				pt.x = g_px;
				pt.y = g_py;
				pt.th = g_pth;
				pt.D = D;
	...
	{	
		...
		if (CalcIdx(start[0], start[1], start[2]))
		{
			...
			// 先检测是否有直接解
			path = AnalysticExpantion(tnode, end); // end
			if (path.lenth)
			{
				PathPoint pt;
				pt.x = start[0];
				pt.y = start[1];
				pt.th = start[2];
				getFinalPath_onlyRS(path, pt);
				FLAG_collison_check_model = 0;
				finish_flag=1;
				std::cout << "direct CCS" << std::endl;
				return true; 
			}
			...
			if (CalcIdx(start_two[0], start_two[1], start_two[2]))
			{
				...
				while (!g_openset.empty() && !g_openset_two.empty())
				{
					...
					path = AnalysticExpantion_CC(g_tnode, temp); // end
					
					if (path.lenth)
					{
						std::cout << "RS start:"<< g_tnode.x << ',' << g_tnode.y << ',' << g_tnode.theta << std::endl;
						std::cout << "RS end:"<<temp[0] << ',' << temp[1] << ',' << temp[2] << std::endl;
						double temp2[3];
						temp2[0]=g_tnode.x;
						temp2[1]=g_tnode.y;
						...
						// 重新对第一段曲线使用HA
						// End2End_HybridA(start, temp2); //temp2
						RsPath empty;
						empty.t=0;
						empty.u=0;
						empty.v=0;
						getFinalPath(empty, str_idx);

						// 获取第二段RS曲线
						PathPoint tmp;
						tmp.x = pathpoint[pathpoint.size() - 1].x;
						tmp.y = pathpoint[pathpoint.size() - 1].y;
						tmp.th = pathpoint[pathpoint.size() - 1].th;
						tmp.D = pathpoint[pathpoint.size() - 1].D;
						tmp.delta = pathpoint[pathpoint.size() - 1].delta;
						getFinalPath_CC(path, tmp);

						// 对第二段用Astar
						// End2End_HybridA(temp2, temp);

						// 直接取第三段曲线作为最终结果
						getFinalPath_onlyAstar(str_idx_two);
						...
						// End2End_HybridA(temp2, end);
						FLAG_collison_check_model = 0;

						auto first_pt = pathpoint[1];
						double d1 = first_pt.D;
						int idx=0;
						for (auto pt: pathpoint)
						{
							if (idx>=2)
							{
								if (pt.D!=d1)
								{
									std::ofstream file("../tmp.txt",std::ios::out | std::ios::trunc);
									// 检查文件是否成功打开
									if (file.is_open()) 
									{
										std::cout << "Switch Pt:"<< pt.x*100+fusion.position.X << ',' 
										<< pt.y*100+fusion.position.Y << ',' << pt.th/M_PI*180 << std::endl;
										file << pt.x*100+fusion.position.X;
										file << ",";
										file << pt.y*100+fusion.position.Y;
										file << ",";
										file << pt.th/M_PI*180;
										file << std::endl;
										// 关闭文件
					...
					// Update(g_tnode, end);
					Update_twoway(g_tnode, g_tnode_two, end, end_two);
	...
	RsPath FindRSPathCC(double x, double y, double phi)
	{
		...
		CC(x, y, phi, rmin, road_yaw);
		for (int i=0;i<6;i++)
		{
			path.road_yaw[i] = road_yaw[i];
		}
		path.lenth = abs(road_yaw[1])+abs(road_yaw[3]);
		return path;
	}
