...
for index_plan = 0
    ...
    while ~feof(fid)
        ...
        elseif log_list == (request-1)*index_L+28
            ...
            gplan_time = num(1);
            aplan_time = num(2);
        elseif (log_list >= (request-1)*index_L+33) && (log_list <= (request-1)*index_L+332)
            num_str = regexp(tline,'\-?\d*\.?\d*','match');
            num = str2double(num_str);
            path(log_list-((request-1)*index_L+33)+1,:) = num;
        end
       %% 
        if (mod(log_list,329) == 0)%||(mod(log_list,327) == 25)
...
%                     plot(X_car(index_x,index_y),Y_car(index_x,index_y),'.','Color',[0.5,0.5,0.5]);hold on;
        ...
        center_x1 = (rect_x+3*offset-0.25) * cosphi - (rect_y-0.28) * sinphi + cpx;
        center_y1 = (rect_x+3*offset-0.25) * sinphi + (rect_y-0.28) * cosphi + cpy;
        ...
        center_x4 = (rect_x-3*offset+0.25) * cosphi - (rect_y-0.28) * sinphi + cpx;
        center_y4 = (rect_x-3*offset+0.25) * sinphi + (rect_y-0.28) * cosphi + cpy;
    
        rect_y=-wid/4;
        center_x5 = (rect_x+3*offset-0.25) * cosphi - (rect_y+0.28) * sinphi + cpx;
        center_y5 = (rect_x+3*offset-0.25) * sinphi + (rect_y+0.28) * cosphi + cpy;
        ...
        center_x8 = (rect_x-3*offset+0.25) * cosphi - (rect_y+0.28) * sinphi + cpx;
        center_y8 = (rect_x-3*offset+0.25) * sinphi + (rect_y+0.28) * cosphi + cpy;
    
        radius = radius8;

        plot(cpx, cpy, 'r.');
        plot(center_x1, center_y1, 'r.');
        plot(center_x5, center_y5, 'r.');
        plot(center_x1 + (radius+0.15) * cos(theta), center_y1 + (radius+0.15) * sin(theta), 'r');
        plot(center_x2 + radius * cos(theta), center_y2 + radius * sin(theta), 'r');
        plot(center_x3 + radius * cos(theta), center_y3 + radius * sin(theta), 'r');
        plot(center_x4 + (radius+0.15) * cos(theta), center_y4 + (radius+0.15) * sin(theta), 'r');
        plot(center_x5 + (radius+0.15) * cos(theta), center_y5 + (radius+0.15) * sin(theta), 'r');
        plot(center_x6 + radius * cos(theta), center_y6 + radius * sin(theta), 'r');
        plot(center_x7 + radius * cos(theta), center_y7 + radius * sin(theta), 'r');
        plot(center_x8 + (radius+0.15) * cos(theta), center_y8 + (radius+0.15) * sin(theta), 'r');
        ...
        plot(center_x1, center_y1, 'r.');
        raidus=radius3;    
        plot(center_x1 + raidus * cos(theta), center_y1 + raidus * sin(theta), 'g');
        plot(center_x2 + raidus * cos(theta), center_y2 + raidus * sin(theta), 'g');
        plot(center_x3 + raidus * cos(theta), center_y3 + raidus * sin(theta), 'g');
